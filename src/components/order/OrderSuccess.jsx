...
export default function OrderSuccess({ form, orderType, orderId, demo = false }) {
  ...
  return (
    <motion.div
      ...
      className="order-form-shell"
    >
      <div className="order-section-card text-center">
        <motion.div
        ...
        <p className="text-warm-gray text-sm sm:text-base leading-relaxed mb-6 max-w-sm mx-auto">
          We&apos;ll contact you to confirm availability, pricing, and{' '}
          {isDelivery ? 'delivery' : 'pickup'} details.
        </p>
        ...
        {orderId && (
          <div className="inline-flex flex-col items-center gap-1 px-6 py-4 rounded-xl bg-cream border border-cream-dark/60 mb-6 w-full">
            <span className="text-xs font-semibold text-warm-gray uppercase tracking-wider">
              Your Order Reference
            </span>
            <span className="font-display text-xl font-semibold text-brown tracking-wide break-all">
              {orderId}
            </span>
